...
> ⚠️ **重要路径说明**：本技能输出到 `data/stocks/` 目录（前端读取），**不是** `data/master/`。
> 
> **服务器环境**：先在远程服务器用 `wechat-article-fetcher` 抓取文章，再用本技能注入个股数据。
> **三端同步**：本地 JSON → GitHub → Firestore（使用 `sync_to_firestore_rest.py` REST API）。
...
### 修复：数据合并路径问题
- **问题**: `pipeline.py` 使用旧版 `incremental_update.py`，将数据写入 skill 内部目录（`.trae/skills/.../data/master/`），前端不读取该路径，导致新数据不显示
- **修复**: `pipeline.py` 现在调用 `merge_new_stocks.py`，自动合并到 `data/stocks/stocks_master.json` 和分片文件
- **验证**: 确保新抓取的文章数据能被前端正确读取
...
```bash
# 服务器端完整工作流（推荐）
python scripts/fetch_article.py "<url>" --output /tmp/articles
python scripts/inject.py  # 手动注入个股数据
python scripts/merge_new_stocks.py --date YYYY-MM-DD
python scripts/sync_to_github.py --mode file --json data/stocks/stocks_master.json
python scripts/sync_to_firestore_rest.py --credentials secrets/firebase-credentials.json --json data/stocks/stocks_master.json
```
...
**Step 1.1: 抓取正文**
```bash
# 文章抓取：使用 wechat-article-fetcher（外部skill），输出 /tmp/articles/
python fetch.py "<url>" --output /tmp/articles --headless true
```

**Step 1.2: 转换为 raw_material 格式**
```bash
# 转换为 raw_material（可选，自动化workflow使用）
python scripts/fetch_wechat_to_raw_material.py \
  --url "<url>" \
  --out "raw_material/raw_material_YYYY-MM-DD.md" \
  --manual_text_file "article.json"
```

**Step 2: 抽取个股信息**
```bash
# 手动注入（推荐）：AI 直接生成 stocks_master_YYYY-MM-DD.json
python inject.py
...
```bash
python scripts/merge_new_stocks.py --data-dir data --date YYYY-MM-DD
```

该脚本会自动：
1. 读取 `data/stocks_master_YYYY-MM-DD.json`（注入产物）
2. 按 `source` 去重合并到 `data/stocks/stocks_master.json`
3. 同步更新 `data/stocks/YYYY-MM-DD.json` 分片文件
4. 累加 `mention_count`
5. **JSON完整性校验**：检测 `<<<<<<<`/`>>>>>>>`/`=======` git 冲突标记

> ⚠️ **v2.4 重要**：读写文件时自动检测 git merge 冲突标记，损坏数据拒绝写入，防止前端崩溃。
...
├── data/
│   ├── stocks_master_YYYY-MM-DD.json (中间产物)
│   ├── stocks/                       (前端读取)
│   │   ├── stocks_index.json         (全局索引)
│   │   ├── stocks_master.json        (主数据文件)
│   │   └── YYYY-MM-DD.json           (分片文件)
│   └── hot_topics/                   [NEW]
│       └── hot_topics_index.json     (热点主题索引)
```
...
python scripts/sync_to_github.py \
  --mode file --json data/stocks/stocks_master.json \
  --remote-path data/stocks/stocks_master.json \
  --github-token "$TOKEN" --github-repo "user/repo"
```
...
| `scripts/sync_to_github.py` | **GitHub 同步**（支持 `--mode file` / `--mode shard`） | v2.4 |
...
| `scripts/sync_to_firestore.py` | 同步到 Firestore（Python SDK，需代理） | v1.x |
| `scripts/sync_to_firestore_rest.py` | ⭐ **Firestore REST API 同步**（稳定，推荐） | v2.4 |
...
### 热点主题索引
`data/hot_topics/hot_topics_index.json`：
```json
{
  "version": "1.0",
  "last_updated": "2026-05-28",
  "topics": {
    "长鑫相关": {"codes": ["688082","688037",...], "count": 19, "created": "2026-05-28"},
    "半导体硅片涨价": {"codes": ["002129","688783",...], "count": 8, "created": "2026-05-28"}
  }
}
```

### 文章标记热点
文章 `articles[]` 可带 `hot_topic` 字段标记所属主题：
```json
{
  "articles": [{
    "source": "...",
...
```
┌─────────────────┐
│ 1. 注入         │ inject.py → stocks_master_YYYY-MM-DD.json
│                 │
│ 2. 合并         │ merge_new_stocks.py → stocks_master.json
│                 │
│ 3. GitHub       │ sync_to_github.py --mode file
│  （hub同步）    │
│                 │
│ 4. Firestore    │ sync_to_firestore_rest.py (REST API)
│  （前端消费）   │
└─────────────────┘
```
...
### 同步前准备
```bash
# 先拉 GitHub 最新 → 合并 → 再推送，避免丢失 IDE 数据
