...
# ========== ARTICLE 3: 6 stocks from XD8d1ouX8r234wujXj9j2w ==========
...
# ========== ARTICLE 4: 12 stocks from CTX26oFNML7WDwCHQ86WIQ ==========
...
# ========== Save update JSONs ==========
print("=" * 60)
print("Creating update JSON files...")
print("=" * 60)

# Article 3 update
update3 = {
    "date": today,
    "update_count": len(article3_stocks),
    "stocks": []
}
for code, sdata in article3_stocks.items():
    entry = {"name": sdata["name"], "code": code, "board": sdata["board"],
        "industry": sdata["industry"], "concepts": sdata["concepts"],
        "products": sdata["products"], "core_business": sdata["core_business"],
        "industry_position": sdata["industry_position"], "chain": sdata["chain"],
        "partners": sdata["partners"], "mention_count": 1,
        "last_updated": today, "articles": sdata["articles"]}
    update3["stocks"].append(entry)

with open("data/stocks_master_2026-06-01_v3.json", "w", encoding="utf-8") as f:
    json.dump(update3, f, ensure_ascii=False, indent=2)
print(f"Article 3 ({len(article3_stocks)} stocks) -> data/stocks_master_2026-06-01_v3.json")

# Article 4 update
update4 = {
    "date": today,
    "update_count": len(article4_stocks),
    "stocks": []
}
for code, sdata in article4_stocks.items():
    entry = {"name": sdata["name"], "code": code, "board": sdata["board"],
        "industry": sdata["industry"], "concepts": sdata["concepts"],
        "products": sdata["products"], "core_business": sdata["core_business"],
        "industry_position": sdata["industry_position"], "chain": sdata["chain"],
        "partners": sdata["partners"], "mention_count": 1,
        "last_updated": today, "articles": sdata["articles"]}
    update4["stocks"].append(entry)

with open("data/stocks_master_2026-06-01_v4.json", "w", encoding="utf-8") as f:
    json.dump(update4, f, ensure_ascii=False, indent=2)
print(f"Article 4 ({len(article4_stocks)} stocks) -> data/stocks_master_2026-06-01_v4.json")

print("\nDone! Now run merge_new_stocks.py twice (v3 first, then v4).")