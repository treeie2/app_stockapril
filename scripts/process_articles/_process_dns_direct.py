...
# -*- coding: utf-8 -*-
"""
直接处理 dns 文件夹文章 → 识别个股 → 生成独立 JSON（无需 LLM API）
"""
...
def find_stocks_in_text(text: str, name_to_code: Dict[str, str]) -> List[Tuple[str, str, str]]:
    ...
    # 模式1: 【代码 名称】 (folders like _000532__多元金融_...)
    ...
    # 模式3: 概念股：名称、名称、名称... (concept chain lists)
    for m in re.finditer(r'概念股[：:]\s*(.*?)(?:\n\n|$)', text, re.DOTALL):
        candidates = re.findall(r'[\u4e00-\u9fa5]{2,6}', m.group(1))
        for candidate in candidates:
            if candidate in name_to_code:
                code = name_to_code[candidate]
                if code not in found:
                    found[code] = (candidate, find_paragraph(text, candidate))

    # 模式4: 单独名称 with signal keywords
...
def extract_item_for_stock(code: str, name: str, context: str, article: dict) -> dict:
    ...
    if context:
        item["article"]["insights"].append(f"[dns] {context[:200]}")
...
def main():
    ...
    raw_md_path = base / "temp11" / "dns_raw_material.md"
    xls_path = base / ".trae" / "skills" / "wechat-fetch-research-embedded" / "assets" / "全部个股.xls"
    standalone_json_path = base / "temp11" / "dns_output.json"
    ...
    for art_idx, article in enumerate(articles, 1):
        ...
        print(f"\r--- 文章 {art_idx}/{len(articles)}: {title[:40]}...", end="")
