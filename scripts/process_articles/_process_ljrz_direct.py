...
# -*- coding: utf-8 -*-
"""
直接处理 ljrz 文章 → 识别个股 → 生成独立 JSON（无需 LLM API）
...
def extract_item_for_stock(
    ...
    if context:
        item["article"]["insights"].append(f"[ljrz] {context[:200]}")
...
def main():
    ...
    raw_md_path = base / "temp11" / "ljrz_raw_material.md"
    xls_path = base / ".trae" / "skills" / "wechat-fetch-research-embedded" / "assets" / "全部个股.xls"
    standalone_json_path = base / "temp11" / "ljrz_output.json"
