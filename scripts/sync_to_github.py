...
def fetch_github_json(github_token: str, github_repo: str, branch: str = "main") -> dict:
    ...
    if response.status_code == 200:
        ...
        # 对于大文件（>1MB），GitHub API 不返回 content，需要用 download_url
        if data.get('encoding') == 'none' or not data.get('content'):
            download_url = data.get('download_url')
            if download_url:
                print(f"   文件较大 ({data.get('size', 0)} bytes)，使用 download_url 获取...")
...
def merge_stocks(existing_stocks: list, new_stocks: list) -> list:
    ...
    for new_stock in new_stocks:
        ...
        if code in stocks_dict:
            ...
            # 更新 last_updated（如果新数据有）
            if new_stock.get('last_updated'):
                existing_stock['last_updated'] = new_stock['last_updated']
        else:
            # 新股票，直接添加
